...
uses
  ...
  cxCustomData, cxStyles, cxTL, cxInplaceContainer, DB, DBTables, DBAccess, Ora, OraSmart, MemDS, OraError,
...
begin
  ...
  for i := 0 to DataGrid.Nodes.Count - 1 do
    begin
       if ( DataGrid.Nodes.AbsoluteItems[i].Values[Col_KirimSec.ItemIndex])  and
          ( VarToInt(DataGrid.Nodes.AbsoluteItems[i].Values[Col_Bos.ItemIndex])>0) then
            begin
                KirimSec.Insert;
                KirimSec.FieldByName('BOS').AsString := VarToStr(DataGrid.Nodes.AbsoluteItems[i].Values[Col_Bos.ItemIndex]);
                KirimSec.Post;
            end;
    end;
...
begin
  DataGrid.FullExpand;
  DataGrid.FocusedNode := DataGrid.Nodes[0];;
  DataGrid.SetFocus;
  Found := False;
  ...
  for i := SonSayac + 1 to DataGrid.Nodes.Count - 1 do
    begin
        Aranan := EdAranan.Text;
        TempStr := VarToStr(DataGrid.Nodes.AbsoluteItems[i].Values[Col_KirimAciklama.ItemIndex]);
        if Pos(Aranan, TempStr) > 0 then
           begin
             DataGrid.FocusedNode := DataGrid.Nodes.AbsoluteItems[i];
...
begin
  ...
  for i := 0 to DataGrid.Nodes.Count -1   do DataGrid.Nodes.AbsoluteItems[i].Values[Col_KirimSec.ItemIndex] := True;
  DataGrid.EndUpdate;
end;
...
begin
  DataGrid.BeginUpdate;
  for i := 0 to DataGrid.Nodes.Count -1   do DataGrid.Nodes.AbsoluteItems[i].Values[Col_KirimSec.ItemIndex] := False;
  DataGrid.EndUpdate;
end;
...
var
  ...
  begin
    for i := 0 to DataGrid.Nodes.Count-1 do
      if (Pos(VarToStr(DataGrid.Nodes[i].Values[col_Kirim.ItemIndex]),S)=1) and
         (Length(VarToStr(DataGrid.Nodes[i].Values[col_Kirim.ItemIndex]))= Length(S)-3) then
         begin
           Result := DataGrid.Nodes[i];
           exit;
         end;
  end;
  ...
  begin
    for i := 0 to DataGrid.Nodes.Count-1 do
      if VarToStr(DataGrid.Nodes[i].Values[col_Kirim.ItemIndex])= S then
         begin
           Result := DataGrid.Nodes[i];
           exit;
         end;
