...
uses
  Consts, Forms, ActiveX, Math, Themes, UxTheme, DwmApi, DBGrids, STdCtrls, DBCtrls, Grids;
...
begin
  ...
  FText := nil;
  UpdateBounds;
  Perform(WM_SETFONT, WPARAM(FFont.Handle), 1);
...
procedure SetPerformingShowingChanged(Instance: TControl); inline;
begin
  Instance.ControlStyle := TControlStyle(Cardinal(Instance.ControlStyle) or csPerformingShowingChanged)
end;

procedure ClearPerformingShowingChanged(Instance: TControl); inline;
begin
  Instance.ControlStyle := TControlStyle(Cardinal(Instance.ControlStyle) and Cardinal(not csPerformingShowingChanged));
end;

function PerformingShowingChanged(Instance: TControl): Boolean; inline;
begin
  Result := Cardinal(Instance.ControlStyle) and csPerformingShowingChanged = csPerformingShowingChanged;
end;
...
begin
  ...
  begin
    ...
    begin
      if FShowing and not PerformingShowingChanged(Self) then
      begin
        SetPerformingShowingChanged(Self);
        try
          FShowing := False;
...
begin
  ...
  if (Self is TCustomDBGrid)or((Self is TCustomEdit)and (TEdit(Self).CharCase = ecUpperCase)) then
    case Key of
      'i' : Key := 'Ý';
      'ý' : Key := 'I';
      'I' : Key := 'I';
  ...
  if Key = #13 then
    begin
      if not((Self is TComboBox) and (TComboBox(Self).DroppedDown)) then
      if not((Self is TDBLookupComboBox) and (TDBLookupComboBox(Self).ListVisible)) then
      if (not (Self is TCustomGrid))and(not (Self is TCustomMemo)) then
...
begin
  inherited;
  if HandleAllocated then Perform(WM_SETFONT, WPARAM(FFont.Handle), 0);
